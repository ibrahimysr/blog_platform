...
@section('content')
	<h1 class="text-2xl font-semibold mb-4">Yeni Etkinlik</h1>
	@if ($errors->any())
	...
	<form method="POST" action="{{ route('admin.events.store') }}" class="space-y-4">
		...
		<div>
			...
			<input name="slug" value="{{ old('slug') }}" class="w-full border rounded px-3 py-2" placeholder="(boş bırakabilirsiniz)">
		</div>
		<div>
			<label class="block text-sm mb-1">Tarih</label>
			<input type="datetime-local" name="event_date" value="{{ old('event_date') }}" class="w-full border rounded px-3 py-2">
		</div>
		<div>
		...
		<div>
			...
			<input name="media_url" value="{{ old('media_url') }}" class="w-full border rounded px-3 py-2">
		</div>
		<div>
			<label class="block text-sm mb-1">Açıklama</label>
			<textarea name="description" class="w-full border rounded px-3 py-2" rows="6">{{ old('description') }}</textarea>
		</div>
