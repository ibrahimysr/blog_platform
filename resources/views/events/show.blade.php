...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-blue-50">
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
		...
		<nav class="flex items-center gap-2 text-sm mb-6 backdrop-blur-sm">
			...
			<a href="{{ route('events.list') }}" class="text-gray-600 hover:text-blue-600 transition-all duration-300 hover:scale-105">Etkinlikler</a>
			<svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
			</svg>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="{{ $hasSidebar ? 'lg:col-span-8' : 'lg:col-span-12' }}">
				...
				<article class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/20 hover:shadow-blue-500/10 transition-all duration-500">
					@if($event->media_url)
						<div class="relative h-[450px] overflow-hidden group">
							...
							<div class="absolute top-6 right-6 bg-white/95 backdrop-blur-md rounded-2xl p-4 shadow-2xl border border-white/50 text-center min-w-[80px] hover:scale-110 transition-transform duration-300">
								...
								<div class="text-xs font-bold text-blue-600 uppercase">{{ $event->event_date?->format('M') }}</div>
					...
					@else
						{{-- No Image Hero --}}
						<div class="relative h-[300px] bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 overflow-hidden">
					...
					<div class="p-8 md:p-12">
						{{-- Quick Info Banner --}}
						<div class="mb-8 p-6 bg-gradient-to-r from-blue-50 via-blue-100 to-blue-200 rounded-2xl border border-blue-200/50 shadow-sm">
						...
						<div class="mb-8">
							...
							<div class="prose prose-lg prose-blue max-w-none prose-headings:font-black prose-p:text-gray-700 prose-p:leading-relaxed">
            ...
            <aside class="lg:col-span-4 space-y-6">
				...
				@if($event->user)
					<div class="sticky top-6 bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/20 hover:shadow-blue-500/10 transition-all duration-500">
						<div class="relative h-32 bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 overflow-hidden">
							<div class="absolute inset-0 opacity-20">
						...
						<div class="relative px-6 pb-6 -mt-16 text-center">
							<div class="w-24 h-24 bg-gradient-to-br from-blue-500 via-blue-600 to-blue-700 rounded-2xl flex items-center justify-center text-white text-3xl font-black mx-auto mb-4 shadow-2xl border-4 border-white transform hover:scale-110 transition-transform duration-300">
							...
							<button class="w-full py-3 px-6 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl font-bold shadow-lg hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 flex items-center justify-center gap-2">
								<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				@if($related->count())
					<div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/20 p-6">
						<h3 class="text-xl font-black text-gray-900 mb-6 flex items-center gap-3">
							<span class="w-1 h-6 bg-gradient-to-b from-blue-600 to-blue-700 rounded-full"></span>
							Benzer Etkinlikler
						</h3>
						<div class="space-y-4">
							@foreach($related as $r)
								<a href="{{ route('events.show', $r) }}" class="group block">
									<div class="flex gap-4 p-3 rounded-xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-blue-100 transition-all duration-300">
										<div class="w-16 h-16 bg-gradient-to-br from-blue-600 to-blue-700 text-white rounded-xl flex flex-col items-center justify-center flex-shrink-0 shadow-lg group-hover:shadow-xl group-hover:scale-105 transition-all duration-300">
